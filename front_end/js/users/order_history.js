...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const paginatedData = await fetchPaginatedDataWithToken(
        'http://127.0.0.1:8000/api/orders/orders/?page_size=1&', 1, access_token
    )
    ...
    data.forEach(order => {
        ...
        orderCard.innerHTML = `
            <div class="order-header">
                ...
                <span class="order-status pending">${now_status.status.name}</span>
            </div>
    ...
    if (entries[0].boundingClientRect.top < window.innerHeight && !isLoading) {
        ...
        debounceTimer = setTimeout( async () => {
            try {
                ...
                const access_token = await getValidAccessToken()
                const paginatedData = await fetchPaginatedDataWithToken('http://127.0.0.1:8000/api/orders/orders/?page_size=1&&', currentPage, access_token);
